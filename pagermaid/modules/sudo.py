...
async def from_msg_get_sudo_id(message: Message) -> int:
...
async def sudo_add(message: Message):
    sudo = get_sudo_list()
    from_id = await from_msg_get_sudo_id(message)
    if from_id in sudo:
        ...
        try:
            ...
            if isinstance(entity, User):
                if entity.deleted:
                    ...
                    sqlite["sudo_list"] = sudo
                    continue
                text += f"• {entity.first_name} - {' '.join(permissions.get_roles_for_user(str(i)))}\n"
            elif isinstance(entity, Channel):
                text += f"• {entity.title} - {' '.join(permissions.get_roles_for_user(str(i)))}\n"
